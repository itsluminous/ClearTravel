...
"Check out my train ticket (PNR 8524567890): https://cleartravel.itsluminous.com/pnr/8524567890";
...
- `am start -a VIEW -d https://cleartravel.itsluminous.com/pnr/8553674906`
...
2. **Real share path** (Files app, `VIEW_DOWNLOADS`, long-press `card-share.png` — the
   ...
   ("Sharing image") lists **Clear Travel**; tapping it → dialog with "Train ticket"
...
| 1 | Relaunch (`force-stop` → start): **"Clear Travel is locked"** unlock screen; wrong password → inline **"That password is not correct."**, still locked; correct password → Trips. `files/security/vault.json` (880 B) is the only key material on disk | PASS | `79-applock-unlock-screen.png`, dumps |
| 1 | **Biometrics (device-tested)**: enrolled a fingerprint on the AVD (`locksettings set-pin` → Settings enrolment → `adb emu finger touch 1` ×12 → "Fingerprint added"); Settings → Security → *Unlock with biometrics* → BiometricPrompt "Confirm to enable biometric unlock" → touch → toggle checked. Cold start → BiometricPrompt **"Unlock Clear Travel"** auto-shown → `finger touch 1` → Trips; wrong finger (`touch 2`) leaves the prompt up; **Use password** → password unlock screen with an "Unlock with biometrics" button → password unlocks. **Lock timing** "Immediately" → Home → relaunch → prompt again → finger → app | PASS | `81-applock-biometric-prompt.png`, dumps |
...
| 3 | **Resume after death**: `force-stop` on step 2 → relaunch → **"Clear Travel is locked"** (the password is asked to *open* the store, never to be *created* again) → `NewDevicePass2` → lands on **Step 2 of 4** directly | PASS | dumps |
...
|---|---|---|---|
| 1a | **Pre-fix reproduction** (APK built from `main` before the fix): fresh install → step 1 → fingerprint Switch `checked=true` (enabled — a print exists) → password + confirm → Create. Landed on **"Clear Travel is locked / Enter your password"** — no BiometricPrompt, and `files/security/vault.json` keys = `salt, iterations, passwordWrap, portableWrap, filesMigrated` (**no `biometricWrap`**). Exactly the user's report. | BUG REPRODUCED | `95-prefix-toggle-on-lands-on-unlock.png`, vault dump |
| 1b | **Fixed build**: same walk → BiometricPrompt **"Confirm to enable fingerprint unlock" / "Skip for now"** shown over step 1 → `finger touch 1` → straight to **Step 2 of 4** (no unlock screen in between); `vault.json` now has **`biometricWrap`**. Use offline → Start fresh → Trips. **Cold start** (`am force-stop` + launch) → BiometricPrompt **"Unlock Clear Travel" / "Use password"** auto-shown → `finger touch 1` → Trips. Settings → Security: *Unlock with biometrics* `checked=true`. | PASS | `96-onboarding-step1-biometric-prompt.png`, `97-coldstart-auto-biometric-prompt.png`, dumps |
...
## Backup fixes + share links — on-device validation + full regression (2026-09-23 10:12–11:25 IST, emulator Android_16_AOSP_Medium, API 36, ADR-038/039/040)

Scope: the four ADR-038 field bugs (Drive folder duplication ×2, fresh-install "No
backups found", boarding pass lost on local restore) and the ADR-039 share links +
flight card quick actions. Fresh install on the AOSP AVD (`pm clear` between phases),
every assertion from `uiautomator dump` text; six blind captures `103`–`108`
(downscaled to 800 px, never viewed). No dumps were taken while the instrumented suite
ran (a second UiAutomation registration crashes the dump and stalls Espresso).

| # | Check | Verdict | Evidence |
|---|---|---|---|
| 1 | **Bug 4 exact scenario.** Wizard (`ValidatePass1`, offline, start fresh) → Journeys/Flights → FAB → *Import boarding pass* → SAF → synthetic `boardingpass.png` (600×300, 6 071 B) → OCR prefilled AI / 101 / ABC123 / 12A / DEL (To misread → BOM, date 2026-10-14 via picker, dep 06:10) → Save → card "AI 101 · DEL → BOM · Wed, 14 Oct 2026", icon *Boarding pass attached*; `files/boarding_passes/1dd4b8cb-….png` (6 104 B, CTEF). Detail sheet → *View boarding pass* → viewer with `Stored document` image `[0,825][1080,1365]` (2:1 = the PNG's aspect). Menu → Backup & Restore → *Export backup* → `cleartravel-backup-20260923-1021.zip` **9 388 B** (CTEB v1 header; pre-fix file-less backups were 4.3 kB) | baseline | dumps |
| 2 | `pm clear` → wizard (`RestorePass2`) → *Use offline* → *Restore from a file* → the 10:21 backup → step 4 → **backup password `ValidatePass1`** → Trips. `files/boarding_passes/1dd4b8cb-….png` is back (6 104 B, different ciphertext = re-encrypted under the new install's key). Flights → AI 101 still *Boarding pass attached* → *View boarding pass* → **viewer renders the image at the same bounds, no "no longer available"**. *Save a copy* → `Download/Boarding pass.png` **md5 `bc8b79ba…` = the original PNG** (byte-identical round trip) | **PASS** | `105-bug4-restored-boarding-pass-viewer.png`, md5 |
| 3 | **Flight card quick actions**: right column content-descs *Check status* / *Web check-in* / *Share flight* on the card. Web check-in on the 14 Oct flight (3 weeks out, AI rule) → snackbar **"Check-in opens 12 Oct, 06:10"** (48 h window, no browser opened) | PASS | `103-flight-card-quick-actions.png`, dump |
| 4 | Share flight → system chooser **"Sharing image"** with the caption `My flight AI 101 · 14 Oct · DEL → BOM - open in Clear Travel to add it: https://cleartravel.itsluminous.com/share/flight/q1Yq…` (199-char link) and the app grid (Messaging, Bluetooth, Print…) | PASS | `104-flight-share-chooser.png`, dump |
| 5 | **Flight link, cold start, https scheme**: `force-stop` → `am start -a VIEW -d <https link> -p com.itsluminous.cleartravel` → **"Clear Travel is locked"** first (ADR-031: nothing decodes before unlock) → password → Add flight form **"Filled from a shared flight link - add your booking reference and seat, then save"**, AI / 101 / 2026-10-14 / DEL / BOM / 6:10, PNR and seat empty (personal fields not shared). Save → **"Flight already exists"** (ADR-025 guard, same flight already on the device). Without `-p` the AVD's default handler (WebView Browser Tester) took the https link — expected until the domain has a landing page / autoVerify (ADR-039 follow-up) | PASS | dumps |
| 6 | **Trip share → link decodes as specified**: trip "ShareTripTokyo" (Sep 25–27, Senso-ji 09:30 Day 1 sight, Tsukiji 12:00 Day 2 food) → detail *Share trip* → chooser "Sharing text" with a 435-char https link; Base64url + raw-inflate off-device → `{"v":1,"id":"320bea7a-…","name":"ShareTripTokyo","startDate":"2026-09-25",…,"items":[{"id":"563dabbd-…","date":"2026-09-25","name":"Senso-ji","plannedTime":"09:30","note":"Oldest","category":"sight"},{…"dayIndex":1,"name":"Tsukiji",…"category":"food"}]}` — original UUIDs, defaults omitted, no journey/calendar ids | PASS | payload |
| 7 | **Trip ADD path** (single device: trip deleted first = tombstone) → `am start VIEW cleartravel://share/trip/<blob>` (warm) → dialog **"Add shared trip?" / `Add trip "ShareTripTokyo" (2 places) to your Trips?`** → *Add* → lands on the trip detail: Day 1 Senso-ji 09:30 Oldest, Day 2 Tsukiji 12:00 (tombstoned row resurrected with its items) | PASS | `106-trip-import-add-dialog.png`, dump |
| 8 | **Trip UPDATE path**: rename Tsukiji → "TsukijiMarket" → share again (444-char link) → open the new link → **"Update your trip?" / `Update your existing trip "ShareTripTokyo" with the shared version (2 places)? Its places will be replaced by the shared ones - places you added yourself will be removed, journey links you made are kept.`** Cancel → open the OLD link → Update → item back to "Tsukiji"; open the NEW link → Update → "TsukijiMarket". Trips list afterwards: **exactly one** "ShareTripTokyo" card | **PASS** | `107-trip-import-update-dialog.png`, dumps |
| 9 | **Checklist round trip incl. check state**: "PackList" from the Medicines preset, Paracetamol ticked ("1 of 12 done" — see #11) → *Share checklist* → 728-char link → delete the checklist → open the link → `Add checklist "PackList" (12 items) to your Checklists?` → Add → **"1 of 12 done", first CheckBox `checked=true`** (the tick survived). Untick Paracetamol, tick Band-aids, rename → "Paracetamol500mg" → share (739 chars) → open → **"Update your checklist?" … "Your check states will be replaced by the shared ones, and items you added yourself will be removed."** Old link → Update → Paracetamol / first box ticked again; new link → Update → "Paracetamol500mg" / third box ticked. One "PackList" in the list | **PASS** | `108-checklist-imported-tick.png`, dumps |
| 10 | **Drive (bugs 1–3)**: NOT device-verified. No Google account is linked on either AVD (Play_36_Pixel booted read-only: Settings → Google account still offers *Connect Google account*; shut down again as found) and linking needs an interactive real-account sign-in. Verified by unit tests (`DriveFolderResolverTest` 8, `DriveBackupServiceTest` +3, `FakeDriveClient`) and code inspection: `findFolders` = `name = … and mimeType = folder and trashed = false`, `orderBy=createdTime`, paginated (`nextPageToken` loop), client tie-break `compareBy { createdAt }.thenBy { folderId }`; `ensureFolder()` under a `Mutex`, cached id trusted only after `convergedThisProcess`; `converge()` moves every file (`addParents/removeParents` PATCH) then trashes the emptied duplicate, any exception → `complete=false` (retry next resolve, canonical still returned); `listBackups()` unions `existingFolderIds()` (never creates), `distinctBy fileId`, name-desc | unit + inspection only | code |
| 11 | **Bug found & fixed in the run (ADR-040)**: after the restore in #2 every built-in preset showed each item TWICE — new "Medicines" checklist **"0 of 12 done"**, Manage presets → Medicines 12 rows, Domestic trip 20 rows. Root cause: seeded built-in preset items had RANDOM ids, so the fresh install's own seed + the backup's rows both survived the ADR-015 by-id merge (pre-existing since backups carried presets; never walked on a device before). Fix: derived seeded ids (`seededItemId(presetId, index)`, v3 UUID) + `collapseSeededBuiltInPresetItems` on import. Re-verified: same pre-fix backup (random ids) → `pm clear` → wizard (`RestorePass3`) → restore → **Medicines 6 rows, Domestic trip 10 rows**, new checklist from Medicines **"0 of 6 done"** | FIXED | dumps |
| 12 | `ktlintCheck lintDebug testDebugUnitTest assembleDebug assembleDebugAndroidTest` → BUILD SUCCESSFUL; unit **1092/1092**, 0 failures (1089 after the two stages + 3 ADR-040 tests) | PASS | `validate3.log` (git-ignored) |
| 13 | `connectedDebugAndroidTest` → **21/21 PASS** on the fixed build (`core:ocr` harness SKIPPED as usual). The first full run hung 15 min in `DocumentsE2eTest.viewer_inLandscape_…` (Espresso `onIdle` never returned after the orientation change) and was reported FAILED by the runner; the class alone passed 4/4 immediately afterwards and two further full runs were 21/21 — treated as an emulator/rotation flake, not a code change | PASS | `connected3.log` |

Notes:

- `adb shell input text` drops spaces (the note "Oldest temple" landed as "Oldest") and
  the onboarding *Create* button sits below the fold once the keyboard has been up —
  swipe the form, do not press BACK twice (the second BACK leaves the app).
- The share-link chooser text is the app's caption + link; the link was lifted from the
  chooser dump and replayed through `am start -a android.intent.action.VIEW -d
